...
- Workbench v1：`项目空间 + 文件库 + 版本 + 建议队列 + 可追溯导出`
...
- 安全层：Provider key 不返回前端；自定义 base URL 默认拦截内网、本机和 link-local 地址；Ollama 需显式允许本地访问
- 事件层：支持 REST 事件列表和 SSE 事件流端点
- UI 层：三栏 Workbench 壳，覆盖文件区、文档预览、版本历史、Agent 面板、建议队列和导出历史
